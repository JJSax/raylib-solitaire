...
namespace solitaire {
    bool suitsCanAlternate(Suit s1, Suit s2) {
    ...
    bool canStackInTableau(const Card& oldTop, const Card& newTop) {
        return (oldTop.face - newTop.face) == 1 && suitsCanAlternate(oldTop.suit, newTop.suit);
    }

    bool canStackInFoundation(Suit foundationSuit, const Card& oldTop, const Card& newTop) {
